...
The simple scenarios now use a three-router, static 0 dBm topology and the runner records one 1 Hz ICMP ping from the mobile end device to its currently observed parent: Router A at `(350, 300)`, Router B at `(750, 300)`, Router C at `(1150, 300)`, and a mobile path from `(350, 360)` to `(1600, 360)`. The mobile is created near Router A, the runner waits until Router A is observed as the parent, and only then are Router B and Router C introduced. During the post-activation settle period, the runner keeps polling the mobile parent so switches before movement sampling are recorded as `pre_movement_switch_observed` rather than hidden as unexpected first samples. OTNS `MeterPerUnit = 0.1` makes the movement path 125 m; 25 one-second movement steps target 5 m/s, followed by a 320 s end dwell. See [`docs/scenarios.md`](docs/scenarios.md).
