# Result: med_static_parent_removal_3routers_static-pps-off / 20260713-134001-run01

Static stock OTNS parent-removal benchmark mirroring the ESPHome stock switch-parent test structure with three router-capable devices and one Minimal End Device. Routers are created first, the child is created after a fixed router-settling period, the child's observed current parent is removed, and the child is observed without mobility.

## Metadata

- Scenario: `med_static_parent_removal_3routers`
- Scenario file: `scenarios/static/med_static_parent_removal_3routers.yaml`
...
- Time spent by parent (s): `{'router_a': 124.0}`
- Configured node TX power (dBm): `{'router_a': 0.0, 'router_b': 0.0, 'router_c': 0.0, 'mobile': 0.0}`
- Verified node TX power (dBm): `{'router_a': 0.0, 'router_b': 0.0, 'router_c': 0.0, 'mobile': 0.0}`
- MLE parent changes: `1`
- MLE attach attempts: `2`
- MLE better parent attach attempts: `0`
- Result classification: `switch_observed`
- Parent ranking events: `4`
- Parent ranking decisions: `{'reject': 3, 'accept': 1}`
- Parent ranking criteria: `{'link_quality_3_count': 2, 'two_way_link_margin': 1, 'partition': 1}`
- Scenario type: `static_parent_removal`
- Router count: `3`
- Parent before removal: `router_b`
- Removed parent: `router_b`
- Parent removal time (s): `305.0`
- Final parent after removal: `router_a`
...
- Ranking CSV: `parent_rank_20260713T134001Z.csv`

## Node Logs

- `node_log_mobile_4.log`
