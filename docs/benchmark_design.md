...
All three use four routers, one mobile end device, straight-line movement, delayed Router B/Router C/Router D activation, static 0 dBm transmit power, and an asymmetric path that starts near Router A. Router A is placed at `(350, 300)`, Router B at `(875, 300)`, Router C at `(1400, 300)`, Router D at `(1925, 300)`, and the mobile path runs from `(350, 360)` to `(2125, 360)`. OTNS `MeterPerUnit = 0.1` makes this a 177.5 m movement path; 36 one-second movement steps keep the path close to the 5 m/s target, followed by a 600 s end dwell. The mobile is created near Router A, Router B/C/D are introduced after a fixed 600 s Router-A-only delay, and movement begins after a monitored 600 s post-activation settle period.

The post-activation settle phase is monitored for parent changes. If the mobile changes parent after Router B/C/D activation but before movement sampling begins, the summary records `pre_movement_parent_events` and classifies the run as `pre_movement_switch_observed` when no later in-window switch is present. This keeps early PPS or attach behavior visible instead of mislabeling the first movement sample as merely unexpected.
